...
NN=rminer::fit(y~.,D[HDS$tr,],model="mlpe",search="heuristic") # 11 time lags
NN1=rminer::fit(y~.,D[HDS$tr,c("lag1","y")],model="mlpe",search="heuristic")

PNN=predict(NN,D[HDS$ts,])
# only 1 input is used, predict wants a data.frame or matrix (10x1):
